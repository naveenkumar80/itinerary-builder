...
export default function ItineraryBuilder() {
  ...
  const generatePDF = () => {
    ...
    const daysHtml = days.map(day => {
      let dayContent = `<div class="day-card"><div class="day-header">Day ${day.dayNumber}: ${day.title}</div>`;
      
      if (day.morning) {
        dayContent += `<div class="time-block"><span class="time-label">🌅 Morning</span><div class="time-content">${day.morning}</div></div>`;
      }
      if (day.afternoon) {
        dayContent += `<div class="time-block"><span class="time-label">☀️ Afternoon</span><div class="time-content">${day.afternoon}</div></div>`;
      }
      if (day.evening) {
        dayContent += `<div class="time-block"><span class="time-label">🌙 Evening</span><div class="time-content">${day.evening}</div></div>`;
      }
      if (day.transport) {
        dayContent += `<div class="transport-info"><strong>🚗 Transport:</strong> ${day.transport}</div>`;
      }
